...
%（以下、文修正案ではなく、コメント）以降の関連研究の流れは2つに分ける。一つ目は（セキュリティではない）ウェブの形式検証の研究で、 [6,7] に相当。まずこれらを説明する。その後に二つ目として、（ウェブではない）セキュリティの研究を紹介、 [5,8,9] に相当。なお、それ以外の内容についてはWeb セキュリティの検証に相当する研究なので、一段落目にて述べる。
...
%ここの分の説明は、Alloy の参考書の記述がベース？おそらく「命題論理の計算が内部処理として走っており、計算に合致しないものを反例として出力する」というようなことを書く（言い方は参考書などを参考に）
...
%ほかに何点か今回新たに実装したところを（中心になるところから順に）掲載する。例えば、次節以降で利用するHTTP レスポンスのヘッダの記述など
...
\end{verbatimtab}
%このあたりもヘッダの記述を記載すること。（それがないと妥当かどうかわからない）
...
%Private オプションが正常に動作しているかの根拠にはならない。前節のレスポンスの格納と再利用はまだ計算結果が一致するという議論から正当性の保証になっているが、これはそもそもそのような比較対象がない。Private オプションがなんであって、どういう記述をしたのかをまず乗せること。
