...
%一方で，情報技術もまた近年目まぐるしい発展を遂げている．
%その発展に応じて従来の技術よりも高速かつ効率的な新しい技術が導入され，ウェブの構造は一層複雑になっている．
%このような状況を受けて，現在広く使われているネットワークの構成要素を含んだウェブの安全性を解析するためのモデルが提案されている．
%この既存モデルは，これからも新しくウェブが発展していく中で安全性を考えていく際に指標となることが期待されている．
Ahkaweら\cite{webmodel}により提案されたモデルは、Web のセキュリティ機能を導入した初のモデルであり、このモデルに基づいた安全性解析が多くの論文で使用されている。
...
%上記の拡張によって、提案モデルはキャッシュを利用した攻撃法であるブラウザキャッシュ汚染攻撃~\cite{bcpattack}を包括できている。
...
また、形式手法を用いた研究として、耐タンパー性を持つソフトウェアの動作を行うハードウェアの検証\cite{specifying_and_verifying_hardware_for}があり、形式手法が脆弱性とその対策に有効であることが示されている。
同様に、OSカーネルの検証\cite{sel4_formal_verification_of_an}が形式手法を用いて行われており、形式手法を安全性証明として利用できることを示している。
...
Ahkaweらによる既存モデル\cite{webmodel}はAlloyを用いて実装が行われており、提案モデルはこれを拡張して実装を進める。

既に実装を終えている要素を図\ref{fig:model_alloy}にまとめる。
これらの要素を用いることでキャッシュによるレスポンスの再利用や有効期限の判定を表現することが可能である。
...
提案モデルの実装は未だ不完全であり、\ref{sec:proposed}章に前述した設計に従って実装を進めている。
\color{black}

\section{事例研究}
%提案モデルの能力評価として、まずキャッシュの基本的な動作であるレスポンスの保存と再利用、経過時間の計算、有効期限の判定、および検証機能を提案モデルで包括しているか確認するため、具体的な事例を取り上げて提案モデルの動作を考察した。

\subsection{レスポンスの格納と再利用}
\color{red}
提案モデルのヘッダ定義が正当であることを確認する。
図\ref{fig:case1}は提案モデルの要件を満たす状態の一例である。

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\hsize]{case1.eps}
\caption{CacheControlヘッダを用いたキャッシュの動作}
\label{fig:case1}
\end{figure}

図\ref{fig:case1}はあるHTTPリクエストに対して、ブラウザキャッシュのような個別キャッシュに格納されているレスポンスを再利用し応答することが可能であることを示している。
このHTTPリクエストのヘッダは以下のように想定されている。
%\begin{small}
\begin{verbatimtab}[4]
	Date: 6
\end{verbatimtab}
...
まず、このレスポンスにはキャッシュを禁止する記述はなく、かつ、有効期限を明記するCache-Controlヘッダのmax-ageオプションが指定されていため、\ref{sec:store_reuse}節のキャッシュへの格納条件を満たしている。
...
有効期限はmax-ageの値が採用され4となり、レスポンスの経過時間を\ref{sec:expiration}節の計算式を用いて求める。
\begin{eqnarray*}
age & = & 7 \\
date & = & 1 \\
current & = & 5 \\
